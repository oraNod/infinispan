...
You do not need to configure a global persistent location for remote caches.
...
You should always enable global state and configure a global persistent location for clustered embedded caches.
...
* Call the `globalState().enable()` method in the `GlobalConfigurationBuilder` API.
+
...
* If the global persistent location is available to all {brandname} instances, specify the `shared-persistent-location` element or call the `sharedPersistentLocation()` method.
+
