...
Any `file-store` configuration is automatically migrated to a `SoftIndexFileStore` after upgrading to Infinispan 13.

To use `SingleFileStore` as a file-based cache store, you must configure it programmatically or include the 12.1 or earlier schema in the declaration as follows:

.Declarative
[source,xml,options="nowrap",subs=attributes+,role="primary"]
----
<persistence>
  <file-store xmlns="urn:infinispan:config:12.1" shared="false" preload="true" fetch-state="true"/>
</persistence>
----
...
However, Infinispan does not recommend using `SingleFileStore` because it does not offer the same performance as `SoftIndexFileStore`.

You must also migrate any `soft-index-file-store` configuration in caches to `file-store` configuration.
...
// What about adding SIF programmatically?
.Programmatic
[source,java,options="nowrap",subs=attributes+,role=""]
----
ConfigurationBuilder b = new ConfigurationBuilder();
b.persistence()
