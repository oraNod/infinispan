...
The following example shows how to create a backup, with all counters and only caches with the names `[cache1,cache2]`,
...
[NOTE]
The backup file must contain a container corresponding to `{cacheManagerName}` in order for the containers content to be restored.
...
multipart/form-data::
Allows the backup zip file to be uploaded to the server with optional JSON parameters.
