[id='configuring-file-based-cache-stores_{context}']
...
Directories are always relative to the global persistent location unless you declare an absolute path.
However, if you set an absolute path that is outside the global persistent location {brandname} throws the following exception:
